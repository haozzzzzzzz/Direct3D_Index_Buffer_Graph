...
void CreateVertexBuffer()
{
	...
	*/
	//创建顶点缓存
	g_pd3dDevice->CreateVertexBuffer(4*sizeof(CUSTOMVERTEX),0,D3DFVF_CUSTOMVERTEX,D3DPOOL_DEFAULT,&g_pVertexBuf,NULL);
	...
	*/
	...
	pVertices[3]=CUSTOMVERTEX(420.0f, 320.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(0,0,255));//V3

	//当使用Lock方法对应缓存区进行加锁并完成数据的访问后，需要调用IDirect3DVertexBuffer9接口的Unlock方法对缓存进行解锁


	//创建索引缓存
	g_pd3dDevice->CreateIndexBuffer(6*sizeof(WORD),0,D3DFMT_INDEX16,D3DPOOL_DEFAULT,&g_pIndexBuf,NULL);
	g_pVertexBuf->Unlock();
	...
	pIndices[3]=1,pIndices[4]=3,pIndices[5]=2;//三角形V1V3V2
	g_pIndexBuf->Unlock();
}
